...
training_dirname = os.path.join(caffe_dirname, 'data/equipboard')
...
if __name__ == '__main__':
    ...
    csv_filename = os.path.join(example_dirname, 'equipboard_category.csv')
    ...
    #     os.path.join(images_dirname, _, ".jpg") for _ in df['id']
    # ]
    df['image_filename'] = [
        os.path.join(images_dirname, "_".join(_.split('/')[5:]).replace(" ", "")) for _ in df['image_url']
    ]
    # Download images.
    num_workers = 1
    if num_workers <= 0:
